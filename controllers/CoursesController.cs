...
public class CoursesController(ICourseService courseService, LinkGenerator linkGenerator) : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        var detail = new CourseDetailDto(
            ...
            new[]
            {
                new LinkDto("self", linkGenerator.GetPathByName(nameof(GetCourseById), new { id }) ?? $"/api/courses/{id}", "GET"),
                new LinkDto("list", linkGenerator.GetPathByName(nameof(GetCourses), values: null) ?? "/api/courses", "GET"),
                new LinkDto("create", "/api/courses", "POST")
            });
